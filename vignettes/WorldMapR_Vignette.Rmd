...
At its bare minimum, this function takes the name of the dataframe, the name of the column with the values to be plotted, the name of the column with the country names. We also add the range of the values that we want to be shown (these should usually be near to the minimum and the maximum observation that we want to plot).
...
- By specifying two or more colours, we obtain a user defined gradient based on the colours we have defined:
...
worldplotCat(data = testdata1c,
             ...
             crs = 3035,
             latitude = c(66, 28), longitude = c(-34, 36),
             transform_limits = TRUE)
```



```{r,fig.width=7, fig.height=5, fig.retina=3, eval=FALSE}
worldplotCat(data = testdata1c,
             ColName = "ValCat",
             CountryName = "iso_a2",
             CountryNameType = "isoa2",
             ...
             transform_limits = FALSE)
```

For additional information regarding the transformation of coordinates in different systems, you can look at https://epsg.io/transform
